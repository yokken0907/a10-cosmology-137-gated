...
enum equation_of_state {CLP,EDE,UA10TX};
...
struct background
{
  ...
  double ua10_current_Gamma; /* 現在のステップにおけるドラッグ強度 */
  ...
  /* some parameters can be used both by background and by
  ...
  double cs2_fld;  /**< \f$ c^2_{s~DE} \f$: sound speed of the fluid in the frame comoving with the fluid (so, this is not [delta p/delta rho] in the synchronous or newtonian gauge!) */
  double Omega_EDE;        /**< \f$ wa_{DE} \f$: Early Dark Energy density parameter */
  double ua10_tx_zt;
  double ua10_tx_sigma;
  double ua10_tx_dw;
...
/* ===== A10-2F-EF ===== */
  ...
  /* input parameters */
  double a10_fede_peak;
  double a10_log10zc;
  double a10_dlogatrig;
  double a10_npost;
  double a10_xi137;
  double a10_gamma0;
  ...
  /* background output indices */
  int index_bg_rho_a10;
  int index_bg_p_a10;
  int index_bg_w_a10;
  int index_bg_rho_drh;
  int index_bg_p_drh;
  int index_bg_S_a10;
  int index_bg_Gamma_a10;

  /* integration vector indices */
  int index_bi_rho_a10;
  int index_bi_rho_drh;
...
/* --- Unified A10 Project Parameters --- */
  short has_unified_a10;           /* モデル起動スイッチ */
  double ua10_phi_ini;             /* 初期値 */
  double ua10_phi_trigger;         /* ゲートの中心（137.0） */
  double ua10_Delta_phi;           /* ゲートの幅 */
  double ua10_V0;                  /* ポテンシャル振幅 */
  double ua10_f_ax;                /* スケール */
  double ua10_n_ax;                /* べき数 */
  double ua10_Gamma0;              /* ドラッグ強度 */

  /* 背景積分のためのインデックス */
  int index_bi_phi;                /* 積分変数phi */
  int index_bi_phidot;             /* 積分変数phidot */

  /* 派生出力のためのインデックス */
  int index_bg_rho_phi;            /* 密度出力用 */
  int index_bg_w_phi;              /* 状態方程式出力用 */
  int index_bg_Gamma_drag;         /* ドラッグ強度出力用 */
  /* -------------------------------------- */
