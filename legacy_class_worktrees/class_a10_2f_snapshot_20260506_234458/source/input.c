...
/* 8.a.2) Equation of state */
  ...
  if (flag1 == _TRUE_) {
    ...
    else if ((strstr(string1,"UA10TX") != NULL) || (strstr(string1,"ua10tx") != NULL)) {
      pba->fluid_equation_of_state = UA10TX;
    }
    else {
...
/* ↓ここが吉村モデルを確実に読み込むための専用配線↓ */
  if (pba->fluid_equation_of_state == UA10TX) {
    class_read_double("ua10_tx_zt",pba->ua10_tx_zt);
    class_read_double("ua10_tx_sigma",pba->ua10_tx_sigma);
    class_read_double("ua10_tx_dw",pba->ua10_tx_dw);
    class_read_double("cs2_fld",pba->cs2_fld);

/* ↓これだけ追加！CLASSの「-1またぎ誤検知」を永久に黙らせるダミー値 */
    pba->w0_fld = -1.01;
  }

  /* A10 block moved to global scope */

}

  /* ===== A10-2F-EF Background Parameters ===== */
  class_read_flag("has_a10_2f",pba->has_a10_2f);

  if (pba->has_a10_2f == _TRUE_) {
    class_read_double("a10_fede_peak",pba->a10_fede_peak);
    class_read_double("a10_log10zc",pba->a10_log10zc);
    class_read_double("a10_dlogatrig",pba->a10_dlogatrig);
    class_read_double("a10_npost",pba->a10_npost);
    class_read_double("a10_xi137",pba->a10_xi137);
    class_read_double("a10_gamma0",pba->a10_gamma0);

    pba->a10_zc = pow(10.,pba->a10_log10zc);
    pba->a10_ac = 1./(1.+pba->a10_zc);
    pba->a10_wn = (pba->a10_npost-1.)/(pba->a10_npost+1.);
    pba->has_drh = _TRUE_;

 /* ===== A10-2F-EF Perturbation Flags ===== */
  ppt->has_perturbations_a10 = pba->has_a10_2f;
  class_read_int("a10_cs_model",ppt->a10_cs_model);
  /* ======================================== */

/* Sanity checks (文字が切れないよう、慎重に上書きしてください) */
    class_test(pba->a10_fede_peak <= 0.0, errmsg, "a10_fede_peak must be > 0");
    class_test(pba->a10_dlogatrig <= 0.0, errmsg, "a10_dlogatrig must be > 0");
    class_test(pba->a10_npost <= 1.0, errmsg, "a10_npost must be > 1");
    class_test(pba->a10_gamma0 < 0.0, errmsg, "a10_gamma0 must be >= 0");
    class_test((pba->a10_xi137 < 130.0 || pba->a10_xi137 > 145.0), errmsg, "a10_xi137 outside sanity range");
  }
  else {
    pba->has_drh = _FALSE_;
  }
...
/* 安全装置：Lambda がマイナスになったらエラーを出して止める */
  class_test(pba->Omega0_lambda < 0.,
             pba->error_message,
             "Factor X (ua10_tx_omega0) is too large, making Omega_lambda negative.");
