...
/* ===== A10-2F-EF Local Helper Functions (定義位置を最上部へ移動) ===== */
static double a10_S_of_a(struct background * pba, double a) {
  double x = (log(a) - log(pba->a10_ac))/pba->a10_dlogatrig;
  return 0.5*(1.0+tanh(x));
}

static double a10_dS_dloga(struct background * pba, double a) {
  double x = (log(a) - log(pba->a10_ac))/pba->a10_dlogatrig;
  double c = cosh(x);
  return 0.5/(pba->a10_dlogatrig*c*c);
}

static double a10_w_of_a(struct background * pba, double a) {
  double S = a10_S_of_a(pba,a);
  return -1.0 + (1.0 + pba->a10_wn)*S;
}

static double a10_Gamma_of_a(struct background * pba, double a) {
  double S = a10_S_of_a(pba,a);
  return pba->a10_gamma0 * pba->a10_Hc * S;
}
...
if (pba->has_a10_2f == _TRUE_) {
    double a       = pvecback[pba->index_bg_a];
    double rho_a10 = pvecback_B[pba->index_bi_rho_a10];
    double rho_drh = pvecback_B[pba->index_bi_rho_drh];
    double w_a10   = a10_w_of_a(pba, a);
    double S       = a10_S_of_a(pba, a);
    double Gamma   = a10_Gamma_of_a(pba, a);

    pvecback[pba->index_bg_rho_a10]   = rho_a10;
    pvecback[pba->index_bg_p_a10]     = w_a10 * rho_a10;
    pvecback[pba->index_bg_w_a10]     = w_a10;
    pvecback[pba->index_bg_rho_drh]   = rho_drh;
    pvecback[pba->index_bg_p_drh]     = rho_drh / 3.0;
    pvecback[pba->index_bg_S_a10]     = S;
    pvecback[pba->index_bg_Gamma_a10] = Gamma;

    pvecback[pba->index_bg_rho_tot] += rho_a10 + rho_drh;
    pvecback[pba->index_bg_p_tot]   += w_a10 * rho_a10 + rho_drh / 3.0;
  }
...
if (pba->has_unified_a10 == _TRUE_) {
    ...
    double f_star = pba->ua10_f_ax; 
    
    /* 局所的ブースト: x * exp(1-x) により z=3000 で最大になり、現代では完全にゼロに消える */
    double bump = x_aline * exp(1.0 - x_aline);
    rho_tot *= (1.0 + f_star * bump);
}

//if (pba->ua10_tx_omega0 > 0.0) rho_tot += ua10_tx_rho(a, pba);
...
/* ================================================================= */
/* ゼロ割りを絶対に起こさない防弾チョッキ付き A10 Factor X 計算モジュール */
/* ================================================================= */
/* --- 吉村 A10 Factor X 計算ユニット（完全版） --- */
static double ua10_tx_y(double a, struct background *pba) {
  if (pba->ua10_tx_sigma <= 0.0) return 0.0;
  double aa = (a > 1e-16) ? a : 1e-16;
  double lna  = log(aa);
  double lnat = -log(1.0 + pba->ua10_tx_zt);
  return (lna - lnat)/pba->ua10_tx_sigma;
}
...
int background_w_fld(
  ...
  /* 全てのケースの初期値を -1.01 に設定（ファントムベースライン） */
  *w_fld = -1.01;
  *dw_over_da_fld = 0.0;
  *integral_fld = 0.0;

  double y, t, s2, lnat, y_today;
  ...
  else if (pba->fluid_equation_of_state == UA10TX) {
    if (pba->ua10_tx_sigma > 0.0) {
      y  = ua10_tx_y(a, pba);
      t  = tanh(y);
      s2 = 1.0 - t*t;
      *w_fld = -1.01 + pba->ua10_tx_dw * s2;
      *dw_over_da_fld = pba->ua10_tx_dw * (-2.0 * s2 * t) / (a * pba->ua10_tx_sigma);
      
      lnat = -log(1.0 + pba->ua10_tx_zt);
      y_today = (0.0 - lnat)/pba->ua10_tx_sigma;
      *integral_fld = 3.0 * pba->ua10_tx_dw * pba->ua10_tx_sigma * (tanh(y_today) - tanh(y));
    }
  }
...
if (pba->has_a10_2f == _TRUE_) {
    pba->index_bg_rho_a10 = index_bg; index_bg++;
    pba->index_bg_p_a10   = index_bg; index_bg++;
    pba->index_bg_w_a10   = index_bg; index_bg++;
    pba->index_bg_rho_drh = index_bg; index_bg++;
    pba->index_bg_p_drh   = index_bg; index_bg++;
    pba->index_bg_S_a10     = index_bg; index_bg++;
    pba->index_bg_Gamma_a10 = index_bg; index_bg++;
  }
...
if (pba->has_a10_2f == _TRUE_) {
    pba->index_bi_rho_a10 = index_bi;
    index_bi++;
    pba->index_bi_rho_drh = index_bi;
    index_bi++;
  }
...
/* ===== A10-2F-EF Analytical Initialization ===== */
  if (pba->has_a10_2f == _TRUE_) {
    /* 1. Calculate Standard Background Density at trigger a_c */
    double rho_bg_ac = (pba->Omega0_g + pba->Omega0_ur) / pow(pba->a10_ac, 4.0)
                     + (pba->Omega0_b + pba->Omega0_cdm) / pow(pba->a10_ac, 3.0);
    rho_bg_ac *= pow(pba->H0, 2.0); /* Convert to physical density units */

    /* 2. Determine target A10 density at a_c from peak fraction */
    double rho_phi_ac = (pba->a10_fede_peak / (1.0 - pba->a10_fede_peak)) * rho_bg_ac;

    /* 3. Backwards Analytic Integration to a_ini (using exact integral of tanh) */
    double integral = 1.5 * (1.0 + pba->a10_wn) * pba->a10_dlogatrig * 0.69314718056; /* 0.693... is ln(2) */
    double rho_a10_ini = rho_phi_ac * exp(integral);

    /* 4. Set Initial Conditions and reference Hubble at trigger */
    pvecback_integration[pba->index_bi_rho_drh] = 0.0;
    pvecback_integration[pba->index_bi_rho_a10] = rho_a10_ini;
    pba->a10_Hc = sqrt((rho_bg_ac + rho_phi_ac) / 3.0);
  }
  /* =============================================== */
...
if (pba->has_a10_2f == _TRUE_) {
    class_store_columntitle(titles, "rho_a10", _TRUE_);
    class_store_columntitle(titles, "p_a10", _TRUE_);
    class_store_columntitle(titles, "w_a10", _TRUE_);
    class_store_columntitle(titles, "rho_drh", _TRUE_);
    class_store_columntitle(titles, "p_drh", _TRUE_);
    class_store_columntitle(titles, "S_a10", _TRUE_);
    class_store_columntitle(titles, "Gamma_a10", _TRUE_);
  }
...
if (pba->has_a10_2f == _TRUE_) {
    ...
    double rho_a10 = y[pba->index_bi_rho_a10];
    double rho_drh = y[pba->index_bi_rho_drh];
    double w_a10   = a10_w_of_a(pba, a);
    double Gamma   = a10_Gamma_of_a(pba, a);

    dy[pba->index_bi_rho_a10] = - a * (3.0 * H * (1.0 + w_a10) * rho_a10 + Gamma * rho_a10);
    dy[pba->index_bi_rho_drh] = - a * (4.0 * H * rho_drh - Gamma * rho_a10);
  }
