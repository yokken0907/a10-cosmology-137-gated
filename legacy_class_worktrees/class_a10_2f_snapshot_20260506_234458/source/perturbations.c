...
int perturbations_vector_init(
  ...
  if (_scalars_) {
    ...
    /* 修正後（そのままコピー＆ペーストしてください） */
    ppv->index_pt_delta_a10 = -1;
    ppv->index_pt_theta_a10 = -1;
    class_define_index(ppv->index_pt_delta_a10, pba->has_a10_2f, index_pt, 1);
    class_define_index(ppv->index_pt_theta_a10, pba->has_a10_2f, index_pt, 1);
  ...
  /* A10-2F-EF indices (Safe Allocation) */
  if (ppt->has_perturbations_a10 == _TRUE_) {
    ppv->index_pt_delta_a10 = index_pt;
    index_pt++;
    ppv->index_pt_theta_a10 = index_pt;
    index_pt++;
  }
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
    ...
    if (_scalars_) {
      ...
      if (pba->has_fld == _TRUE_) {
        ...
        if (ppt->has_perturbations_a10 == _TRUE_) {
          ppw->pv->y[ppw->pv->index_pt_delta_a10] = 0.0;
          ppw->pv->y[ppw->pv->index_pt_theta_a10] = 0.0;
        }
        else {
...
/* 👇👇 ここから下の5行をそのままコピー＆ペーストして挿入 👇👇 */
      /* === A10-2F-EF initial conditions (安全装置) === */
  ...
  if (_scalars_) {
    ...
    if (pba->has_dr == _TRUE_) {
      ...
      */
      ...
      ppw->
delta_rho += rho_dr_over_f*y[ppw->pv->index_pt_F0_dr];
   ...
   /*if (ppt->has_perturbations_a10 == _TRUE_) {
      double rho_a10_sources = ppw->pvecback[pba->index_bg_rho_a10];
      double p_a10_sources   = ppw->pvecback[pba->index_bg_p_a10];
      double cs2_a10_sources = (ppt->a10_cs_model == 1) ? pba->a10_wn : 1.0;
      ppw->delta_rho += rho_a10_sources * y[ppw->pv->index_pt_delta_a10];
      ppw->delta_p   += cs2_a10_sources * rho_a10_sources * y[ppw->pv->index_pt_delta_a10];
      ppw->rho_plus_p_theta += (rho_a10_sources + p_a10_sources) * y[ppw->pv->index_pt_theta_a10];
    }
...
/* 👇👇 ここから下の5行をそのままコピー＆ペーストして挿入 👇👇 */
    /* === A10-2F-EF dynamics (安全装置) === */
    if (pba->has_a10_2f == _TRUE_) {
        dy[pv->index_pt_delta_a10] = 0.0;
        dy[pv->index_pt_theta_a10] = 0.0;
    }
/* 👆👆 挿入ここまで 👆👆 */
...
/* A10-2F-EF perturbation equations */
   /* if (ppt->has_perturbations_a10 == _TRUE_) {
      double a_a10 = pvecback[pba->index_bg_a];
      double calH_a10 = pvecback[pba->index_bg_H] * a_a10;
      double w_a10 = pvecback[pba->index_bg_w_a10];
      double Gamma_a10 = pvecback[pba->index_bg_Gamma_a10];
      double cs2_a10 = (ppt->a10_cs_model == 1) ? pba->a10_wn : 1.0;
      double onepw_a10 = 1.0 + w_a10;
      if (fabs(onepw_a10) < 1e-8) onepw_a10 = 1e-8;

      dy[ppw->pv->index_pt_delta_a10] = -(onepw_a10) * (y[ppw->pv->index_pt_theta_a10] + metric_continuity) 
                                   - 3.0 * calH_a10 * (cs2_a10 - w_a10) * y[ppw->pv->index_pt_delta_a10]
                                   - a_a10 * Gamma_a10 * y[ppw->pv->index_pt_delta_a10];
      dy[ppw->pv->index_pt_theta_a10] = -calH_a10 * (1.0 - 3.0 * cs2_a10) * y[ppw->pv->index_pt_theta_a10]
                                   + (cs2_a10 / onepw_a10) * (k*k) * y[ppw->pv->index_pt_delta_a10]
                                   + metric_euler - a_a10 * Gamma_a10 * y[ppw->pv->index_pt_theta_a10];
    }*/
