...
/* ===== A10: Gaussian-window continuous injection model ===== */
  short has_a10;

  double a10_z_trigger;       /* e.g. 5000 */
  double a10_loga_trigger;    /* log(a_trigger) = log(1/(1+z_trigger)) */
  double a10_sigma_loga;      /* Gaussian width in log(a) */
  double a10_gamma_peak;      /* transfer strength */
  double a10_wY;              /* usually 1/3 */
  double a10_wX;              /* usually 1   */
  double a10_frac_y_ini;      /* rho_Y(a_ini) / rho_r(a_ini) */

  /* indices of dynamical background variables */
  int index_bi_rho_a10_y;
  int index_bi_rho_a10_x;

  /* indices for background output vector */
  int index_bg_rho_a10_y;
  int index_bg_rho_a10_x;
  int index_bg_Omega_a10_y;
  int index_bg_Omega_a10_x;
  int index_bg_a10_q_over_h;
  int index_bg_a10_window;
  /* =========================================================== */
  ...
  int index_bg_rho_Y;   /* メーター用 Y */
  int index_bg_rho_X;   /* メーター用 X */
...
/* --- 137プロジェクト専用：座席名簿 --- */
  int index_bi_chi;     /* スカラー場 chi の座席 */
  int index_bi_u_chi;   /* chi の速度の座席 */
  int index_bi_rho_Y;   /* 親成分 Y の密度の座席 */
  int index_bi_rho_X;   /* 娘成分 X の密度の座席 */
