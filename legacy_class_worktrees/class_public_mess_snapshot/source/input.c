...
int input_read_from_file(struct file_content * pfc,
  class_read_double("z_trigger", pba->a10_z_trigger);
  class_read_double("a10_frac_y_ini", pba->a10_frac_y_ini);
  class_read_double("a10_gamma_peak", pba->a10_gamma_peak);
  if (pba->a10_z_trigger > 0) pba->a10_loga_trigger = log(1.0 / (1.0 + pba->a10_z_trigger));
                         struct precision * ppr,
                         struct background *pba,
                         struct thermodynamics *pth,
                         struct perturbations *ppt,
