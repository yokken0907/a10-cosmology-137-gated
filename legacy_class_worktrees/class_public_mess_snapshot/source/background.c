...
/* --- A10: Helper Functions --- */
int background_a10_window(struct background * pba, double a, double * W) {
  double loga = log(a);
  double arg = (loga - pba->a10_loga_trigger) / pba->a10_sigma_loga;
  *W = exp(-0.5 * arg * arg);
  return _SUCCESS_;
}

int background_a10_q_over_h(struct background * pba, double a, double rho_y, double * q_over_h, double * W_out) {
  double W;
  background_a10_window(pba, a, &W);
  *q_over_h = pba->a10_gamma_peak * W * rho_y;
  if (W_out != NULL) {
    *W_out = W;
  }
  return _SUCCESS_;
...
/* --- A10: 計算結果を出力用メモリに書き写す（コピー係） --- */
  if (pba->has_a10 == _TRUE_) {
    /* 積分された密度をコピー */
    pvecback[pba->index_bg_rho_a10_y] = pvecback_B[pba->index_bi_rho_a10_y];
    pvecback[pba->index_bg_rho_a10_x] = pvecback_B[pba->index_bi_rho_a10_x];
  }
...
/* --- A10: Background Functions (Friedmann) --- */
if (pba->has_a10 == _TRUE_) {
  double rho_a10_y = pvecback_B[pba->index_bi_rho_a10_y];
  double rho_a10_x = pvecback_B[pba->index_bi_rho_a10_x];
  double q_over_h, W;

  background_a10_q_over_h(pba, a, rho_a10_y, &q_over_h, &W);

    rho_tot += rho_a10_y + rho_a10_x;
    p_tot   += pba->a10_wY * rho_a10_y + pba->a10_wX * rho_a10_x;

    pvecback[pba->index_bg_rho_a10_y] = rho_a10_y;
    pvecback[pba->index_bg_rho_a10_x] = rho_a10_x;
    pvecback[pba->index_bg_Omega_a10_y] = rho_a10_y / rho_tot;
    pvecback[pba->index_bg_Omega_a10_x] = rho_a10_x / rho_tot;
    pvecback[pba->index_bg_a10_q_over_h] = q_over_h;
    pvecback[pba->index_bg_a10_window] = W;
  }
...
/* --- A10: 出力用の Omega (密度比) を計算 --- */
  if (pba->has_a10 == _TRUE_) {
    pvecback[pba->index_bg_Omega_a10_y] = pvecback[pba->index_bg_rho_a10_y] / pvecback[pba->index_bg_rho_crit];
    pvecback[pba->index_bg_Omega_a10_x] = pvecback[pba->index_bg_rho_a10_x] / pvecback[pba->index_bg_rho_crit];

    /* ※ window と q_over_h は、もし微分方程式の中で pba->... に保存しているならここで代入 */
    /* pvecback[pba->index_bg_a10_q_over_h] = pba->a10_q_over_h; */
    /* pvecback[pba->index_bg_a10_window] = pba->a10_window; */
  }
...
/* --- 1. A10の出力用インデックスを定義（国境線の前！） --- */
  if (pba->has_a10 == _TRUE_) {
    class_define_index(pba->index_bg_rho_a10_y, _TRUE_, index_bg, 1);
    class_define_index(pba->index_bg_rho_a10_x, _TRUE_, index_bg, 1);
    class_define_index(pba->index_bg_Omega_a10_y, _TRUE_, index_bg, 1);
    class_define_index(pba->index_bg_Omega_a10_x, _TRUE_, index_bg, 1);
    class_define_index(pba->index_bg_a10_q_over_h, _TRUE_, index_bg, 1);
    class_define_index(pba->index_bg_a10_window, _TRUE_, index_bg, 1);
  }
...
/* --- A10: Indices --- */
  if (pba->has_a10 == _TRUE_) {
    /* 内部積分用（bi）のメモリ確保：先にカウントを増やす */
    class_define_index(pba->index_bi_rho_a10_y, _TRUE_, index_bi, 1);
    class_define_index(pba->index_bi_rho_a10_x, _TRUE_, index_bi, 1);
  }
...
/* --- A10: Initial Conditions --- */
if (pba->has_a10 == _TRUE_) {
  /* 引数で渡された実際の初期スケールファクター 'a' を使う！ */
  double rho_r_analytic = (pba->Omega0_g + pba->Omega0_ur) * (pba->H0 * pba->H0) / (a * a * a * a);
  
  pvecback_integration[pba->index_bi_rho_a10_y] = pba->a10_frac_y_ini * rho_r_analytic;
  pvecback_integration[pba->index_bi_rho_a10_x] = 0.0;

    /* 🎯 【着火確認用メーター】ターミナルに初期値を印字！ */
    printf("\n============================================\n");
    printf(" [A10 IGNITION SUCCESS!] rho_Y initial = %e\n", pvecback_integration[pba->index_bi_rho_a10_y]);
    printf("============================================\n\n");
  }
...
/* --- A10: Output Titles --- */
  if (pba->has_a10 == _TRUE_) {
    class_store_columntitle(titles,"(.)rho_a10_y",_TRUE_);
    class_store_columntitle(titles,"(.)rho_a10_x",_TRUE_);
    class_store_columntitle(titles,"(.)Omega_a10_y",_TRUE_);
    class_store_columntitle(titles,"(.)Omega_a10_x",_TRUE_);
    class_store_columntitle(titles,"(.)a10_q_over_h",_TRUE_);
    class_store_columntitle(titles,"(.)a10_window",_TRUE_);
  }
...
/* --- A10: Output Data --- */
  if (pba->has_a10 == _TRUE_) {
    class_store_double(dataptr, pvecback[pba->index_bg_rho_a10_y], _TRUE_, storeidx);
    class_store_double(dataptr, pvecback[pba->index_bg_rho_a10_x], _TRUE_, storeidx);
    class_store_double(dataptr, pvecback[pba->index_bg_Omega_a10_y], _TRUE_, storeidx);
    class_store_double(dataptr, pvecback[pba->index_bg_Omega_a10_x], _TRUE_, storeidx);
    class_store_double(dataptr, pvecback[pba->index_bg_a10_q_over_h], _TRUE_, storeidx);
    class_store_double(dataptr, pvecback[pba->index_bg_a10_window], _TRUE_, storeidx);
  }
...
/* --- A10: Differential Equations --- */
  if (pba->has_a10 == _TRUE_) {
    double rho_a10_y = y[pba->index_bi_rho_a10_y];
    double rho_a10_x = y[pba->index_bi_rho_a10_x];
    double q_over_h, W;

    /* 微小値の保護（クリッピング）：値が小さすぎる場合はゼロに固定 */
    if (rho_a10_y < 1e-30) rho_a10_y = 0.0;
    if (rho_a10_x < 1e-30) rho_a10_x = 0.0;

    background_a10_q_over_h(pba, a, rho_a10_y, &q_over_h, &W);

    /* 独立変数が loga なので、そのままの数式で正しい */
    double dy_y = -3.0 * (1.0 + pba->a10_wY) * rho_a10_y - q_over_h;
    double dy_x = -3.0 * (1.0 + pba->a10_wX) * rho_a10_x + q_over_h;

    /* 負の密度へ突入するのを防ぐ最終ガード */
    if (rho_a10_y <= 0.0 && dy_y < 0.0) dy_y = 0.0;
    if (rho_a10_x <= 0.0 && dy_x < 0.0) dy_x = 0.0;

    dy[pba->index_bi_rho_a10_y] = dy_y;
    dy[pba->index_bi_rho_a10_x] = dy_x;
  }
