...
OUTDIR="chains"
mkdir -p "$OUTDIR"
SUMMARY="$OUTDIR/p2_unified_refine_scan.tsv"
...
# Refined sweep around the apparent sweet spot near 1e-10
V0_VALUES=(1e-12 3e-12 1e-11 3e-11 1e-10 3e-10 1e-9)
...
make_yaml() {
  local v0="$1"
  local tag="${v0//./p}"
  local yaml="p2_unified_refine_V0_${tag}.yaml"

  cat > "$yaml" <<YAML
output: ${OUTDIR}/p2_unified_refine_V0_${tag}
