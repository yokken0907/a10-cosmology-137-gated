...
OUTDIR="chains"
mkdir -p "$OUTDIR"
SUMMARY="$OUTDIR/toggle_matrix_summary.tsv"

# Point definitions:
# name H0 omega_b omega_cdm A_s n_s tau_reio ua10_f_ax ua10_V0 ua10_Gamma0 ua10_phi_trigger
POINTS=(
  "P0_baseline68 68.206 0.0223 0.1194 2.1e-09 0.965 0.055 0.1 1.0 1.0 137.47"
  "P1_controlC 68.237676 0.022362701 0.11942085 2.1e-09 0.965 0.055 0.12 1558634.018 0.016 137.47"
  "P2_legacyA73 72.999169 0.023437102 0.10867378 2.1207673e-09 0.99455865 0.071083465 0.11552662 5000.0166 0.047755317 137.90903"
)

SWITCHES=("OFF" "ON")
V0_VALUES=("0" "1e-16" "1e-4")

printf "case\tswitch\tua10_V0_override\tstatus\tloglike\tchi2_CMB\tchi2_SN\tchi2_shoes\terror_hint\n" > "$SUMMARY"
...
make_yaml() {
  ...
  local tau_reio="$9"
  local ua10_f_ax="${10}"
  local ua10_Gamma0="${11}"
  local ua10_phi_trigger="${12}"

  local prefix="toggle_${case_name}_${switch}_V0_${v0_override//./p}"
  local yaml="$prefix.yaml"
...
for point in "${POINTS[@]}"; do
  read -r case_name H0 omega_b omega_cdm A_s n_s tau_reio ua10_f_ax ua10_V0_base ua10_Gamma0 ua10_phi_trigger <<< "$point"
  for switch in "${SWITCHES[@]}"; do
    for v0 in "${V0_VALUES[@]}"; do
      yaml=$(make_yaml "$case_name" "$switch" "$v0" "$H0" "$omega_b" "$omega_cdm" "$A_s" "$n_s" "$tau_reio" "$ua10_f_ax" "$ua10_Gamma0" "$ua10_phi_trigger")
      prefix="${yaml%.yaml}"
      ...
      if [[ "$status" == "ok" ]]; then
        ...
        error_hint=$(grep -E 'Step size too small|Class did not read input parameter|NameError|CosmoComputationError|CosmoSevereError' "$stdout_log" | tail -n 1 | sed 's/\t/ /g' || true)
      fi
      printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
        "$case_name" "$switch" "$v0" "$status" "$loglike" "$chi2cmb" "$chi2sn" "$chi2shoes" "$error_hint" >> "$SUMMARY"
    done
  done
done
