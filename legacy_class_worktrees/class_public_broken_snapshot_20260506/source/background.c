...
int background_functions(
  ...
  pvecback[pba->index_bg_chi]   = pvecback_B[pba->index_bi_chi];
  pvecback[pba->index_bg_u_chi] = pvecback_B[pba->index_bi_u_chi];
  pvecback[pba->index_bg_rho_Y] = pvecback_B[pba->index_bi_rho_Y];
...
/* 137プロジェクト：出力用の座席（必ず bg_size 確定の直前に入れる！） */
  class_define_index(pba->index_bg_chi,   _TRUE_, index_bg, 1);
  class_define_index(pba->index_bg_u_chi, _TRUE_, index_bg, 1);
  class_define_index(pba->index_bg_rho_Y, _TRUE_, index_bg, 1);
  class_define_index(pba->index_bg_rho_X, _TRUE_, index_bg, 1);

  /* -> end of indices in the great table of background quantities */
  pba->bg_size = index_bg; /* ← ここでサイズが正しく拡張されます */
...
/* --- 積分変数の座席予約（掟に従った修正版） --- */
  ...
  /* 標準的なエネルギー成分 */
  class_define_index(pba->index_bi_rho_g,   _TRUE_, index_bi, 1);
  class_define_index(pba->index_bi_rho_b,   _TRUE_, index_bi, 1);
  class_define_index(pba->index_bi_rho_cdm, _TRUE_, index_bi, 1);
  class_define_index(pba->index_bi_rho_ur,  _TRUE_, index_bi, 1);
  /* (中略：他の rho 成分があればここに入れる) */

  /* 137プロジェクト：成分グループ(B)の最後に配置 */
  class_define_index(pba->index_bi_chi,   _TRUE_, index_bi, 1);
  class_define_index(pba->index_bi_u_chi, _TRUE_, index_bi, 1);
  class_define_index(pba->index_bi_rho_Y, _TRUE_, index_bi, 1);
  class_define_index(pba->index_bi_rho_X, _TRUE_, index_bi, 1);
...
/* 137プロジェクト：計算結果を破棄せずファイルに保存するよう命じる */
  used_in_output[pba->index_bi_chi]   = _TRUE_;
  used_in_output[pba->index_bi_u_chi] = _TRUE_;
  used_in_output[pba->index_bi_rho_Y] = _TRUE_;
  used_in_output[pba->index_bi_rho_X] = _TRUE_;
...
/* 137プロジェクト：正規のメモリ位置への初期値セット */
  pvecback_integration[pba->index_bi_chi]   = 0.0;
  pvecback_integration[pba->index_bi_u_chi] = 0.0;
  pvecback_integration[pba->index_bi_rho_Y] = 1.0;
...
/* 137プロジェクト：正規の流儀で列の名前を末尾に「追加」する */
  class_store_columntitle(titles, "chi",   _TRUE_);
  class_store_columntitle(titles, "u_chi", _TRUE_);
  class_store_columntitle(titles, "rho_Y", _TRUE_);
  class_store_columntitle(titles, "rho_X", _TRUE_);
...
/* 〇 137プロジェクト：CLASS正規のマクロを使って安全に順次書き込む */
  class_store_double(data, pvecback[pba->index_bg_chi],   _TRUE_, storeidx);
  class_store_double(data, pvecback[pba->index_bg_u_chi], _TRUE_, storeidx);
  class_store_double(data, pvecback[pba->index_bg_rho_Y], _TRUE_, storeidx);
  class_store_double(data, pvecback[pba->index_bg_rho_X], _TRUE_, storeidx);
...
/* 137プロジェクト：検証フェーズ1（ゼロ・テスト版） */
  {
    /* 変数の読み込み（一応読み込みますが、使いません） */
    double chi_val = y[pba->index_bi_chi];
    double u_chi_val = y[pba->index_bi_u_chi];
    double rho_Y_val = y[pba->index_bi_rho_Y];
    double rho_X_val = y[pba->index_bi_rho_X];

    /* すべての変化を強制的にゼロにする */
    dy[pba->index_bi_chi]   = 0.0;
    dy[pba->index_bi_u_chi] = 0.0;
    dy[pba->index_bi_rho_Y] = 0.0;
    dy[pba->index_bi_rho_X] = 0.0;
  } /* ← ここでカッコを閉じる */

  return _SUCCESS_; /* 関数の最後は必ずこれ */
} /* background_derivs 関数の終わりのカッコ */
